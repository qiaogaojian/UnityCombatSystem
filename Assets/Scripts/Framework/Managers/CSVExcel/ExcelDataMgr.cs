...
public class ExcelDataMgr
{
	...
	public static ExcelDataMgr Instance = null;

	public void Init() {
		ExcelDataMgr.Instance = this;
	...
	public void ReadConfigData(Type setT, string fileName = null, bool hasPriKey = true)
	{
		if (fileName == null) {
			fileName = setT.Name;
		}
		...
		for (int colNum = 1; colNum < csr.ColCount + 1; colNum++)
		{
			fis[colNum - 1] = setT.GetField(csr[3, colNum]);
		}

		int index = 0;
		for (int rowNum = 4; rowNum < csr.RowCount + 1; rowNum++)
		{
			object configObj = Activator.CreateInstance(setT);
			for (int i = 0; i < fis.Length; i++)
			{
				string fieldValue = csr[rowNum, i + 1];
				object setValue = new object();
